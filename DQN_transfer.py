...
def get_screen():
    return resize(np.float32(game.board)).unsqueeze(0).to(device)
...
while i_episode < num_episodes:
    ...
    game = Tetris()
    done = game.generate_block(choice=random.randint(0,3)) # Juste carré début
    state = get_screen()
    total_reward = 0
    ...
    for t in count():
        ...
        if game.block_reached_end():
            sparsity = sum(np.where(game.board[0:(game.nb_rows-2)].sum(axis=1) > 1,1,0))/10
        ...
        if done:
            ...
            if i_episode % mod_i == mod_i-1 and i_episode >= mod_i-1:
                ...
                torch.save(model_dict,'/content/drive/My Drive/model/smaller' + datetime.now().strftime("%Y-%m-%d %H-%M") + '.pth')
                time.sleep(0.1)
#            print(game)
            break
