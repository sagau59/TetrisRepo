...
n_actions = 4
policy_net = DQN(screen_height, screen_width, n_actions,layers=1).to(device)
target_net = DQN(screen_height, screen_width, n_actions,layers=1).to(device)
PATH = 'C:/Users/sagau/Google Drive/model1_2.pth'
...
episode_squares = 1000000

while i_episode < num_episodes:
    # Initialize the environment and state
    game = Tetris()
    if i_episode < episode_squares:
    ...
    for t in count():
        ...
        if game.block_reached_end():
            sparsity = sum(np.where(game.board[0:(15-2)].sum(axis=1) > 1,1,0))/10
